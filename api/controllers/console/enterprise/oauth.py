...
class EnterpriseOAuthLogin(Resource):
    def get(self):
        OAUTH_PROVIDERS = get_oauth_providers()
        with current_app.app_context():
            oauth_provider = OAUTH_PROVIDERS.get('google')
...
class EnterpriseOAuthCallback(Resource):
    def get(self):
        ...
        try:
            ...
            logging.exception(
                f"An error occurred during the OAuth process with google: {e.response.text}")
            return {'error': 'OAuth process failed'}, 400

        account = _generate_account('google', user_info)
...
api.add_resource(EnterpriseOAuthLogin, '/enterprise/sso/oauth2/login')
api.add_resource(EnterpriseOAuthCallback, '/enterprise/sso/oauth2/authorize')
